...
- Pathfinder v1: new storage method `fileTroubleHistory` and MCP tool `task_file_history` expose per-file claim/takeover/conflict counts in a 14-day window. `task_ready_for_agent` ready entries gain a `pathfinder_file_history` array when scope overlaps with troubled files, so an agent can see "past lanes lost ownership on file X" before claiming.
