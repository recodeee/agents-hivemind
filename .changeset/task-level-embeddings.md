...
(`task_suggest_approach`); subsequent PRs build similarity search,
pattern extraction, and the surface tools on top of it.

`@colony/storage`:

- New `task_embeddings` table (schema version 9). One row per task with
  `(task_id, model, dim, vec, observation_count, computed_at)`.
